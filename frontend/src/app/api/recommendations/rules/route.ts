 ...
 *
 * Proxies rules catalog request to recommendations service
 * Returns published crop rotation rules including botanical families and predecessors
 */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const backendResponse = await fetch(
      `${recommendationsServiceUrl}/recommendations/rules`,
      {
        method: 'GET',
        headers: {
    ...
    if (!backendResponse.ok) {
      ...
      const errorResponse: ErrorResponse = {
        ...
        error: {
          code: errorData.code || 'rules_error',
          detail: errorData.detail || 'Failed to get rules catalog',
        },
